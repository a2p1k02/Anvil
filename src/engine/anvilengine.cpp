...
void anvilengine::createDescriptorSetLayout()
{
    ...
    if (vkCreateDescriptorSetLayout(vkDevice, &layoutCreateInfo, nullptr, &descriptorSetLayout) != VK_SUCCESS) {
        throw std::runtime_error("failed to create descriptor set layout!");
    }
}

void anvilengine::createUniformBuffers()
{
    VkDeviceSize bufferSize = sizeof(anvilutils::UniformBufferObject);
...
void anvilengine::createDescriptorPool()
{
    ...
    if (vkCreateDescriptorPool(vkDevice, &poolCreateInfo, nullptr, &descriptorPool) != VK_SUCCESS) {
        throw std::runtime_error("failed to create descriptor pool!");
    }
}

void anvilengine::createDescriptorSets()
{
    ...
    if (vkAllocateDescriptorSets(vkDevice, &allocateInfo, descriptorSets.data()) != VK_SUCCESS) {
    ...
    for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
        ...
        bufferInfo.offset = 0;
        bufferInfo.range = sizeof(anvilutils::UniformBufferObject);
...
void anvilengine::createIndexBuffer()
{
...
void anvilengine::createVertexBuffer()
{
...
void anvilengine::createSyncObjects()
{
    ...
    for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
        if (vkCreateSemaphore(vkDevice, &semaphoreCreateInfo, nullptr, &imageAvailableSemaphores[i]) != VK_SUCCESS ||
...
void anvilengine::recordCommandBuffer(VkCommandBuffer commandBuffer, uint32_t imageIndex)
{
    ...
    VkClearValue clearColor = {{{0.05f, 0.1f, 1.f, 1.f}}};
...
void anvilengine::createCommandBuffers()
{
...
void anvilengine::createCommandPool()
{
    anvilutils::QueueFamilyIndices queueFamilyIndices = findQueueFamilies(physicalDevice);
...
void anvilengine::createGraphicsPipeline()
{
    ...
    auto bindingDescription = anvilutils::Vertex::getBindingDescription();
    auto attributeDescriptions = anvilutils::Vertex::getAttributeDescriptions();
...
{
    anvilutils::QueueFamilyIndices indices = findQueueFamilies(physicalDevice);
...
{
    anvilutils::SwapChainSupportDetails swapChainSupport = querySwapChainSupport(physicalDevice);
...
{
    anvilutils::QueueFamilyIndices indices = findQueueFamilies(device);
    bool extensionSupported = checkDeviceExtensionSupport(device);

    bool swapChaiAdequate = false;
    if (extensionSupported) {
        anvilutils::SwapChainSupportDetails swapChainSupport = querySwapChainSupport(device);
...
anvilutils::QueueFamilyIndices anvilengine::findQueueFamilies(VkPhysicalDevice device)
{
    anvilutils::QueueFamilyIndices indices;
...
anvilutils::SwapChainSupportDetails anvilengine::querySwapChainSupport(VkPhysicalDevice device)
{
    anvilutils::SwapChainSupportDetails details;
...
uint32_t anvilengine::findMemoryType(uint32_t typeFilter, VkMemoryPropertyFlags properties)
{
...
void anvilengine::createBuffer(VkDeviceSize size, VkBufferUsageFlags usage, VkMemoryPropertyFlags properties, VkBuffer& buffer, VkDeviceMemory& bufferMemory)
{
...
void anvilengine::copyBuffer(VkBuffer srcBuffer, VkBuffer dstBuffer, VkDeviceSize size)
{
...
void anvilengine::updateUniformBuffer(uint32_t currentImage)
{
